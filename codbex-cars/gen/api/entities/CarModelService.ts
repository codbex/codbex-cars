...
class CarModelService {
    ...
    private readonly repository = new CarModelRepository();

    @Get("/")
    public getAll(_: any, ctx: any) {
        try {
            const options: CarModelEntityOptions = {
    ...
    public getById(_: any, ctx: any) {
        try {
            ...
            if (entity) {
                ...
                HttpUtils.sendResponseNotFound("CarModel not found");
            }
        } catch (error: any) {
            this.handleError(error);
        }
    ...
    private validateEntity(entity: any): void {
        ...
        if (entity.Model?.length > 255) {
            throw new ValidationError(`The 'Model' exceeds the maximum length of [255] characters`);
        }
